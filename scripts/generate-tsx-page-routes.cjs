...
for (const file of routeFiles) {
  ...
  const rawImportMatch = content.match(
    /import\s+raw\s+from\s+"\.\.\/\.\.\/pages\/([^\"]+)\?raw"/,
  );
  if (!rawImportMatch) continue;
  const htmlFile = rawImportMatch[1];
...
const marketingRoute = "service-marketing.tsx";
if (
  !generatedFiles.has(marketingRoute) &&
  !fs.existsSync(path.join(routesDir, marketingRoute))
) {
  createRouteFile({
    routePath: "/service-marketing",
    htmlFile: "service-marketing.html",
    outputFile: marketingRoute,
  });
}
