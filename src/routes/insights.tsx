...
export const Route = createFileRoute("/insights")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Expert insights on cybersecurity, cloud computing, ERP, AI integration and IT strategy from Dastute Technologies. Case studies, industry analysis and thought leadership." },
      { property: "og:title", content: "Insights & Blog — Dastute Technologies" },
      { property: "og:description", content: "Cybersecurity analysis, cloud strategy, ERP insights, AI integration guides and IT industry thought leadership." },
...
type Category = "All" | "Cybersecurity" | "IT Strategy" | "Development" | "Case Study";
...
const CATEGORIES: Category[] = ["All", "Cybersecurity", "IT Strategy", "Development", "Case Study"];
  ...
  return (
    <SiteLayout>
      ...
      <section className="px-6 py-24 md:py-32 max-w-7xl mx-auto">
        <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary block mb-6">/ Insights</span>
          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 md:divide-x divide-border">
            <div className="p-12 group hover:bg-foreground hover:text-background transition-colors duration-500">
              <div className="flex justify-between items-start mb-16">
                <span className="font-mono text-[10px] uppercase tracking-widest text-primary group-hover:text-background/60">Featured</span>
                <span className="font-mono text-[10px] text-muted-foreground group-hover:text-background/50">{featuredArticle.date}</span>
              </div>
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-6">{featuredArticle.title}</h2>
              <p className="text-muted-foreground group-hover:text-background/70 mb-8 leading-relaxed">{featuredArticle.excerpt}</p>
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-[10px] text-muted-foreground group-hover:text-background/50">{featuredArticle.readTime}</span>
              </div>
            </div>

            <div className="p-12 flex flex-col justify-between border-t md:border-t-0 border-border">
              <div>
                <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground block mb-8">Latest by Category</span>
                <div className="space-y-6">
                  {["Cybersecurity", "IT Strategy", "Case Study"].map((cat) => {
                    const latest = ARTICLES.find((a) => a.category === cat);
                    return latest ? (
                      <div key={cat} className="border-b border-border pb-6 last:border-0 last:pb-0">
                        <span className="font-mono text-[10px] text-primary uppercase">{cat}</span>
                        <h4 className="font-bold mt-2 mb-1">{latest.title}</h4>
                        <span className="text-sm text-muted-foreground">{latest.date} · {latest.readTime}</span>
                      </div>
                    ) : null;
                  })}
                </div>
              ...
              <div className="mt-12 pt-8 border-t border-border">
                <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground block mb-4">Stay Updated</span>
                <p className="text-sm text-muted-foreground mb-4">IT strategy and cybersecurity insights, delivered to your inbox. No spam.</p>
                <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); alert("Thanks — you're on the list."); }}>
                  <input
                    type="email"
                  ...
                  />
                  <button type="submit" className="bg-foreground text-background px-5 py-2 text-[10px] font-bold uppercase tracking-widest hover:bg-primary transition-colors">
                    Subscribe
                  </button>
                </form>
              </div>
      ...
      <section className="px-6 py-24 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16">
          <h3 className="text-xs font-mono uppercase tracking-[0.2em] text-muted-foreground">All Articles</h3>
          ...
          {filtered.map((article) => (
            ...
            >
              <div className="grid md:grid-cols-[160px_1fr_200px] gap-6 md:gap-12 items-start">
                <div>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-primary group-hover:text-background/60 block mb-2">{article.category}</span>
                  <span className="font-mono text-[10px] text-muted-foreground group-hover:text-background/50">{article.date}</span>
                </div>
                <div>
                  <h3 className="text-xl md:text-2xl font-bold tracking-tight mb-3">{article.title}</h3>
                  <p className="text-sm text-muted-foreground group-hover:text-background/70 leading-relaxed mb-4">{article.excerpt}</p>
                  <div className="flex flex-wrap gap-2">
                    {article.tags.map((t) => (
                      <span key={t} className="px-3 py-1 border border-current/20 rounded-full text-[10px] uppercase">{t}</span>
                    ))}
                ...
                <div className="flex md:flex-col md:items-end justify-between md:justify-start gap-4">
                  <span className="font-mono text-[10px] text-muted-foreground group-hover:text-background/50">{article.readTime}</span>
      ...
      <section className="px-6 pb-24 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-end">
          <div>
            <h3 className="text-3xl md:text-4xl font-bold tracking-tight">Have an IT challenge you'd like us to cover?</h3>
            <p className="mt-4 text-muted-foreground max-w-md">
              We write about the problems our clients face. If there's a cybersecurity, cloud or IT strategy challenge on your mind, let us know.
            </p>
          </div>
          <div className="flex md:justify-end gap-4">
            <Link to="/contact" className="inline-flex items-center gap-2 border border-foreground px-6 py-3 text-sm font-medium hover:bg-foreground hover:text-background transition-colors">
