...
export const Route = createFileRoute("/marketing")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Result-driven digital marketing (SEO, PPC, Social Media, Email), Brand Strategy, Brand Identity systems and Product Film visualization from Dastute Technologies." },
      { property: "og:title", content: "Brand & Digital Marketing Services — Dastute Technologies" },
      { property: "og:description", content: "Comprehensive brand strategy and full-funnel digital marketing services from our UK and India teams." },
  ...
  return (
    <SiteLayout>
      ...
      <section className="px-6 py-24 md:py-32 max-w-7xl mx-auto">
        <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary block mb-6">/ Creative Practice</span>
        <h1 className="text-5xl md:text-7xl font-bold tracking-tight leading-[0.95] text-balance max-w-4xl">
          Positioning, campaigns<br />and full-funnel growth.
        </h1>
        <p className="mt-8 max-w-xl text-muted-foreground text-lg">
          Five creative and marketing disciplines designed to help you stand out, drive search traffic, and capture leads globally.
        </p>
      ...
      <section className="border-y border-border">
        <div className="max-w-7xl mx-auto">
          {SERVICES.map((s, i) => (
            <article
              key={s.code}
              className={`grid md:grid-cols-[160px_1fr_1fr] gap-8 md:gap-12 p-8 md:p-12 ${i !== 0 ? "border-t border-border" : ""}`}
            >
              <div>
              ...
              <div>
                <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground block mb-4">Deliverables</span>
      ...
      <section className="px-6 py-24 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-end">
          <div>
            <h3 className="text-3xl md:text-4xl font-bold tracking-tight">Need technical infrastructure too?</h3>
            <p className="mt-4 text-muted-foreground max-w-md">
              Our technology practice handles cloud architecture, cybersecurity, managed IT and custom software engineering.
            </p>
          </div>
          <div className="flex md:justify-end gap-4">
            <Link to="/technology" className="inline-flex items-center gap-2 border border-foreground px-6 py-3 text-sm font-medium hover:bg-foreground hover:text-background transition-colors">
